<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-extrabold text-xl sm:text-2xl text-slate-900 leading-tight truncate mr-3">
                ...
                <span class="hidden sm:inline">Edit Produk: </span>{{ $product->nama }}
            </h2>
            <a href="{{ route('admin.products.index') }}" class="shrink-0 text-xs font-bold text-slate-600 hover:text-slate-900 flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-xl transition">
    ...
    <div class="py-6 sm:py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm space-y-6">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div x-data="{ 
                        barcode: '{{ old('barcode', $product->barcode ?? 'HKT-' . rand(100000, 999999)) }}',
                        generateNew() {
                        ...
                        <p class="text-[11px] text-slate-400 mt-1">Klik <b>Auto Generate</b> untuk buat kode acak baru atau ubah manual jika perlu.</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $product->nama) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">
                        @error('nama') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Merk / Produsen</label>
                        <input type="text" name="merk" value="{{ old('merk', $product->merk) }}" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Harga (Rp) *</label>
                        <input type="number" name="harga" value="{{ old('harga', $product->harga) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">
                    ...
                    <div class="md:col-span-2 bg-slate-50 p-5 rounded-2xl border border-slate-200" x-data="{ 
                        showMapModal: false, 
                        areaName: '{{ old('lokasi_area', Str::before($product->lokasi_rak, ' - ')) }}',
                        rowDetail: '{{ old('detail_baris', Str::contains($product->lokasi_rak, ' - ') ? Str::after($product->lokasi_rak, ' - ') : '') }}',
                        fullLocation: '{{ old('lokasi_rak', $product->lokasi_rak) }}',
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">
                            ...
                            <option value="Stok Habis" {{ $product->status == 'Stok Habis' ? 'selected' : '' }}>Stok Habis</option>
                        </select>
                    </div>
                </div>
                ...
                <x-photo-input
                    ...
                    label="Foto Produk 📸"
                    accent-color="brand"
                    :current-src="$product->foto ? asset('storage/' . $product->foto) : null"
                />
                        ...
                        @foreach($mainCategories as $mainCat)
                            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                <label class="inline-flex items-center gap-2 text-xs font-extrabold text-slate-900 cursor-pointer">
                                    <input type="checkbox" name="categories[]" value="{{ $mainCat->id }}" {{ $product->categories->contains($mainCat->id) ? 'checked' : '' }} class="rounded text-brand-600 focus:ring-brand-500 w-4 h-4">
                                    ...
                                    <div class="mt-2 ml-6 grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2 border-t border-slate-100">
                                        @foreach($mainCat->children as $sub)
                                            <label class="inline-flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                                                <input type="checkbox" name="categories[]" value="{{ $sub->id }}" {{ $product->categories->contains($sub->id) ? 'checked' : '' }} class="rounded text-emerald-600 focus:ring-emerald-500 w-3.5 h-3.5">
                                                <span>└─ {{ $sub->nama }}</span>
                                            </label>
                                        @endforeach
                                    </div>
                ...
                <div x-data="{
                    searchQuery: '',
                    selectedType: 'all',
                    selectedCount: {{ $product->problems->count() }},
                    updateCount() {
                        ...
                        @foreach($problems as $prob)
                            <label x-show="(selectedType === 'all' || '{{ strtolower($prob->tipe) }}' === selectedType) && (!searchQuery || '{{ strtolower($prob->nama) }}'.includes(searchQuery.toLowerCase()) || '{{ strtolower($prob->tipe) }}'.includes(searchQuery.toLowerCase()))"
                                ...
                                <div class="flex items-center gap-2.5">
                                    <input type="checkbox" name="problems[]" value="{{ $prob->id }}" {{ $product->problems->contains($prob->id) ? 'checked' : '' }} @change="updateCount()" class="rounded text-amber-600 focus:ring-amber-500 w-4 h-4">
                ...
                <div>
                    ...
                    <input type="text" name="bahan_aktif" value="{{ old('bahan_aktif', $product->bahan_aktif) }}" placeholder="Contoh: Abamektin 18 g/l, Klorpirifos 200 g/l" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-500 text-sm font-medium">{{ old('deskripsi', $product->deskripsi) }}</textarea>
                </div>
                ...
                <div class="pt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-3 border-t border-slate-100">
                    ...
                    <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-6 py-3 rounded-xl shadow-lg">Perbarui Produk</button>
                </div>
            </form>
        </div>
    </div>
